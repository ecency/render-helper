...
function markdownToHTML(input, forApp, webp) {
    ...
    input = input.replace("https://inleo.io/threads/view/", "/@");
    ...
    var md = new Remarkable({
        ...
        highlight: function (str) {
            try {
                ...
                return tokens.map(function (token) { return "<span class=\"ll-".concat(token[0], "\">").concat(token[1], "</span>"); }).join('');
            }
            catch (err) {
                console.error(err);
            }
    ...
    try {
        ...
        var doc = consts_1.DOMParser.parseFromString("<body id=\"root\">".concat(output, "</body>"), 'text/html');
        (0, traverse_method_1.traverse)(doc, forApp, 0, webp);
    ...
    output = output.replace(/ xmlns="http:\/\/www.w3.org\/1999\/xhtml"/g, '')
        ...
        .replace('</body>', '')
        .trim();
    return (0, sanitize_html_method_1.sanitizeHtml)(output);
}
