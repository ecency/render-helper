...
function iframe(el) {
    ...
    if (m && m.length === 2) {
        var s = "https://player.vimeo.com/video/".concat(m[1]);
        el.setAttribute('src', s);
        return;
    }
    // Twitch
    if (src.match(consts_1.TWITCH_EMBED_REGEX)) {
        ...
        var s = "".concat(src, "&parent=").concat(parentDomain, "&autoplay=false");
        el.setAttribute('src', s);
        return;
    }
    // 3Speak
    if (src.match(consts_1.SPEAK_EMBED_REGEX)) {
        var normalizedSrc = src.replace(/3speak\.[a-z]+/i, '3speak.tv');
        var hasAutoplay = /[?&]autoplay=/.test(normalizedSrc);
        var s = hasAutoplay
            ? normalizedSrc
            : "".concat(normalizedSrc).concat(normalizedSrc.includes('?') ? '&' : '?', "autoplay=true");
        el.setAttribute('src', s);
        return;
    }
    // Spotify
    ...
    if (src.match(consts_1.SOUNDCLOUD_EMBED_REGEX)) {
        ...
        if (match && match.length === 2) {
            var s = "https://w.soundcloud.com/player/?url=".concat(match[1], "&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&visual=true");
            el.setAttribute('src', s);
            return;
        }
    }
    ...
    // IPFS Skatehive
    if (src.match(consts_1.SKATEHIVE_IPFS_REGEX)) {
        el.setAttribute('src', src);
        el.setAttribute('allowfullscreen', 'true');
        return;
    }
    ...
    // Loom
    if (src.match(consts_1.LOOM_EMBED_REGEX)) {
        el.setAttribute('src', src);
        el.setAttribute('frameborder', '0');
        return;
    }
    // Aureal
    if (src.match(consts_1.AUREAL_EMBED_REGEX)) {
        el.setAttribute('src', src);
        el.setAttribute('frameborder', '0');
        return;
    }
    ...
    replaceNode.textContent = "(Unsupported ".concat(src, ")");
    if (el.parentNode) {
