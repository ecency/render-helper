...
function linkify(content, forApp, webp) {
    ...
    content = content.replace(/((^|\s)(\/|)@[\w.\d-]+)\/(\S+)/gi, function (match, u, p1, p2, p3) {
        ...
        if (consts_1.SECTION_LIST.some(function (v) { return p3.includes(v); })) {
            var attrs = forApp ? "https://ecency.com/@".concat(uu, "/").concat(perm) : "href=\"/@".concat(uu, "/").concat(perm, "\"");
            return " <a class=\"markdown-profile-link\" ".concat(attrs, ">@").concat(uu, "/").concat(perm, "</a>");
        }
        else {
            var attrs = forApp ? "data-author=\"".concat(uu, "\" data-tag=\"post\" data-permlink=\"").concat(perm, "\"") : "href=\"/post/@".concat(uu, "/").concat(perm, "\"");
            return " <a class=\"markdown-post-link\" ".concat(attrs, ">@").concat(uu, "/").concat(perm, "</a>");
        }
    });
    // Image links
