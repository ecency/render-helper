...
function a(el, forApp, webp) {
    ...
    if ((tpostMatch && consts_1.WHITE_LIST.includes(tpostMatch[1].substring(1))) || (tpostMatch && tpostMatch.length === 4 && tpostMatch[1].indexOf('/') !== 0)) {
        if (['wallet', 'feed', 'followers', 'following', 'points', 'communities', 'posts', 'blog', 'comments', 'replies', 'settings', 'engine'].includes(tpostMatch[3])) {
            ...
            el.setAttribute('class', 'markdown-post-link');
            var tag = 'post';
            if (!consts_1.WHITE_LIST.includes(tpostMatch[1].substring(1))) {
                tag = tpostMatch[1];
            }
    ...
    if (forApp) {
        ...
        var externalRegex = /(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
