...
function text(node, forApp, webp) {
    ...
    var linkified = (0, linkify_method_1.linkify)(nodeValue, forApp, webp);
    ...
    if (nodeValue.match(consts_1.IMG_REGEX)) {
        var isLCP = false; // Traverse handles LCP; no need to double-count
        var imageHTML = (0, img_method_1.createImageHTML)(nodeValue, isLCP, webp);
        var replaceNode = consts_1.DOMParser.parseFromString(imageHTML);
        node.parentNode.replaceChild(replaceNode, node);
    }
    // If a youtube video
    if (nodeValue.match(consts_1.YOUTUBE_REGEX)) {
        var e = consts_1.YOUTUBE_REGEX.exec(nodeValue);
        if (e[1]) {
            var vid = e[1];
            ...
            var startTime = (0, helper_1.extractYtStartTime)(nodeValue);
            if (startTime) {
